...
thetr.event.listen = function(args) {
  var event;
  if (thetr.event.eventsOn[args.on.thetrObjectId] &&
      thetr.event.eventsOn[args.on.thetrObjectId][args.action]) {
    event = thetr.event.eventsOn[args.on.thetrObjectId][args.action];
    event.addHandler(args);
    //console.log( 'added to existing');
  } else {
    event = new thetr.event.Event(args);
    thetr.event.eventsById[event.id] = event;
    args.on.thetrObjectId = thetr.event.nextObjectId++;
    if (!thetr.event.eventsOn[args.on.thetrObjectId]) {
      thetr.event.eventsOn[args.on.thetrObjectId] = {};
    }
    thetr.event.eventsOn[args.on.thetrObjectId][args.action] = event;
    event.attach();
...
thetr.event.unlisten = function(args) {
  ...
  if (thetr.event.eventsOn[args.on.thetrObjectId] &&
      thetr.event.eventsOn[args.on.thetrObjectId][args.action]) {
    var event = thetr.event.eventsOn[args.on.thetrObjectId][args.action];
    event.removeHandler(args);
  }
};
...
thetr.event.Event.prototype.removeHandler = function(args) {
  if (args.handler) {
    var curHandler = this.handlersByHandler[args.handler];
    for (var iter = 0, hler; hler = this.handlers[iter]; iter++) {
      if (hler == curHandler) {
        this.handlers.splice(iter, 1);
        break;
      }
    }
  }
